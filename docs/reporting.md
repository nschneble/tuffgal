...
| `diffPixels`                           | number?        | Count of differing pixels. One of the two pixel gates: an action passes only when this is at or under its `diff.maxDiffPixels` budget (default `0`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `diffRatio`                            | number?        | `diffPixels / totalPixels`, `0`–`1`. Absent when dimensions mismatched (no diff could be computed).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `ssimScore`                            | number?        | Mean structural similarity, `0`–`1`. `1.0` = identical. The other pixel gate: an action passes only when this is at or above its `diff.ssimThreshold` AND `diffPixels` is within budget. In CI mode a drifted accessibility tree also flips a pixel-passing action to `changed` (see `a11yChanged`).                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `a11yDiff`                             | object?        | Unified line diff of those two snapshots, `{ lines, added, removed, truncated }`. Present whenever `a11yChanged` is `true`, absent on older results. `lines` are prefixed (`' '` unchanged, `-` removed, `+` added), elided stretches marked `…`; `added` / `removed` count the whole diff even when `lines` is clipped.                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `history`                              | array?         | Trailing series of this action+breakpoint's `diffPixels`/`diffRatio` across recent runs, `[{ finishedAt, diffPixels, diffRatio }]`, oldest to newest, this run's own value last. Present only on a `pass`/`changed` action with a computable diff; absent for `new`/`failed`/`skipped` actions and an uncomputable size-mismatch diff. Read from and written to a mode-appropriate history store (the per-machine `paths.localCache/history.json` in local mode, the committed `paths.baselines/history.json` in CI mode); see [the CI-owned baselines model](ci.md#ci-owns-the-baselines) for why CI-mode history only persists forward on the next `approve --from`, same as the environment manifest. Capped to the last 20 runs per action+breakpoint. |
